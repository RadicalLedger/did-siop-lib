  ...
  sign(message: string, key: RSAKey, algorithm: ALGORITHMS): Buffer {
    if (key.isPrivate()) {
      ...
      switch (algorithm) {
        ...
        default:
          throw new Error(ERRORS.INVALID_ALGORITHM);
      }
...
export class ECSigner extends Signer {
  ...
  sign(message: string, key: ECKey, algorithm: ALGORITHMS): Buffer {
    if (key.isPrivate()) {
      ...
      Buffer.from(leftpad(ec256k_signature.r.toString("hex")), "hex").copy(
        signature,
        0
      );
      Buffer.from(leftpad(ec256k_signature.s.toString("hex")), "hex").copy(
        signature,
        32
      );
...
export class ES256KRecoverableSigner extends Signer {
  ...
  sign(message: string, key: ECKey | string): Buffer {
    ...
    if (
      ec256k_signature.recoveryParam !== undefined &&
      ec256k_signature.recoveryParam !== null
    )
