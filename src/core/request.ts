...
async function validateRequestParams(
  ...
  if (
    parsed.url !== "openid://" ||
    !parsed.query.client_id ||
    parsed.query.client_id.toString().match(/^ *$/) ||
    !parsed.query.response_type ||
    parsed.query.response_type.toString().match(/^ *$/)
  )
  ...
  if (parsed.query.scope) {
    ...
    if (
      !requestedScopes.every((s) => op_metadata.scopes.includes(s)) ||
      !REQUIRED_SCOPES.every((s) => requestedScopes.includes(s))
    )
      return Promise.reject(ERROR_RESPONSES.invalid_scope.err);
  } else return Promise.reject(ERROR_RESPONSES.invalid_request.err);

  if (
    !op_metadata.response_types.includes(parsed.query.response_type.toString())
  )
    return Promise.reject(ERROR_RESPONSES.unsupported_response_type.err);

  if (parsed.query.request === undefined || parsed.query.request === null) {
    if (
      parsed.query.request_uri === undefined ||
      parsed.query.request_uri === null
    ) {
      return Promise.reject(ERROR_RESPONSES.invalid_request.err);
    } else {
      ...
      try {
        ...
        return returnedValue.data
          ? returnedValue.data
          : Promise.reject(ERROR_RESPONSES.invalid_request_uri.err);
      } catch (err) {
        return Promise.reject(ERROR_RESPONSES.invalid_request_uri.err);
      }
    }
...
async function validateRequestJWT(
  ...
  if (
    ...
    try {
      ...
      try {
        ...
        if (decodedPayload.jwks) {
          ...
          decodedPayload.jwks_uri &&
          decodedPayload.jwks_uri ===
            RESOLVER_URL + decodedPayload.iss + ";transform-keys=jwks"
        ) {
    ...
    try {
      await validateRequestJWTClaims(decodedPayload);
    } catch (err) {
