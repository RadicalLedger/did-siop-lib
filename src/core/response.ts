...
export class DidSiopResponse {
  ...
  static async generateResponse(
    requestPayload: any,
    signingInfo: JWT.SigningInfo,
    didSiopUser: Identity,
    ...
    try {
      ...
      if (
        requestPayload.registration.id_token_signed_response_alg.includes(
          ALGORITHMS[signingInfo.alg]
        )
      ) {
      ...
      let didPubKey = keys.find((authKey) => {
      ...
      if (requestPayload.redirect_uri)
        payload.aud = requestPayload.redirect_uri; // Changed as per SIOPV2
      ...
      try {
        ...
        return Promise.reject(
          ERROR_RESPONSES.vp_token_missing_presentation_definition.response
            .error
        );
      }
   ...
   */
  static async generateResponseWithVPData(
    requestPayload: any,
    signingInfo: JWT.SigningInfo,
    didSiopUser: Identity,
    expiresIn: number = 1000,
    ...
    try {
      id_token_s = await this.generateResponse(
        requestPayload,
        signingInfo,
        didSiopUser,
        expiresIn,
      ...
      if (vps && vps.vp_token) {
        ...
        vp_token_s = await this.generateResponseVPToken(
          requestPayload,
          signingInfo,
          vps
        ); // Generate VP Token
   ...
   * https://identity.foundation/did-siop/#generate-siop-response
   */
  static async generateResponseVPToken(
    requestPayload: any,
    signingInfo: JWT.SigningInfo,
    vps?: VPData
  ): Promise<string> {
  ...
  static async validateResponse(
  ...
  ): Promise<JWT.JWTObject | ErrorResponse.SIOPErrorResponse> {
    ...
    if (
      decodedHeader.kid &&
      !decodedHeader.kid.match(/^ *$/) &&
      decodedPayload.iss &&
      !decodedPayload.iss.match(/^ *$/) &&
      decodedPayload.aud &&
      !decodedPayload.aud.match(/^ *$/) &&
      decodedPayload.did &&
      !decodedPayload.did.match(/^ *$/) &&
      decodedPayload.sub &&
      !decodedPayload.sub.match(/^ *$/) &&
      decodedPayload.sub_jwk &&
      !JSON.stringify(decodedPayload.sub_jwk).match(/^ *$/)
    ) {
      if (decodedPayload.iss !== "https://self-issued.me")
        return Promise.reject(new Error(ERRORS.NON_SIOP_FLOW));

      if (decodedPayload.aud !== checkParams.redirect_uri)
        return Promise.reject(new Error(ERRORS.INCORRECT_AUDIENCE));

      if (decodedPayload.nonce && decodedPayload.nonce !== checkParams.nonce)
        return Promise.reject(new Error(ERRORS.INCORRECT_NONCE));

      if (checkParams.validBefore) {
        if (decodedPayload.iat) {
          if (decodedPayload.iat + checkParams.validBefore <= Date.now())
            return Promise.reject(new Error(ERRORS.JWT_VALIDITY_EXPIRED));
        } else {
      ...
      if (jwkThumbprint !== decodedPayload.sub)
        return Promise.reject(new Error(ERRORS.INVALID_JWK_THUMBPRINT));

      let publicKeyInfo: JWT.SigningInfo | undefined;
      try {
        let identity = new Identity();
        if (resolvers) identity.addResolvers(resolvers);
  ...
  static async validateResponseWithVPData(
    tokensEncoded: SIOPTokensEcoded,
    checkParams: CheckParams,
    resolvers?: DidResolver[]
  ): Promise<SIOPTokenObjects | ErrorResponse.SIOPErrorResponse> {
    try {
      let decodedIDToken = await this.validateResponse(
        tokensEncoded.id_token,
        checkParams,
        resolvers
      );
