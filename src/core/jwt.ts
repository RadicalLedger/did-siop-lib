...
export const ERRORS = Object.freeze({
  ...
  ALGORITHM_MISMATCH:
    "Algorithm in jwt header does not match alg in signing info",
...
export function sign(jwtObject: JWTObject, signingInfo: SigningInfo): string {
  ...
  let algorithm: ALGORITHMS =
    ALGORITHMS[jwtObject.header.alg as keyof typeof ALGORITHMS];
...
export function verify(jwt: string, signingInfo: SigningInfo): boolean {
  ...
  let algorithm: ALGORITHMS =
    ALGORITHMS[decoded.header.alg as keyof typeof ALGORITHMS];
  if (algorithm !== signingInfo.alg) throw new Error(ERRORS.ALGORITHM_MISMATCH);
...
export function isJWTObject(obj: any): boolean {
  return "header" in obj && "payload" in obj;
}
