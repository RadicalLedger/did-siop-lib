 ...
 */
export async function validateRequestJWTClaims(
  decodedPayload: any
): Promise<any> {
  if (decodedPayload.claims) {
    ...
    if (decodedPayload.claims.vp_token) {
      ...
      if (decodedPayload.claims.vp_token.presentation_definition) {
        if (
          !validJsonObject(
            decodedPayload.claims.vp_token.presentation_definition
          )
        )
          return Promise.reject(ERROR_RESPONSES.invalid_vp_token.err);
      } else {
        ...
        ); // if vp_token exists,the presentation_definition must exist
      }
    }
  }
...
export async function validateResponseVPTokenJWT(
  vp_tokenJWT: JWTObject
): Promise<any> {
