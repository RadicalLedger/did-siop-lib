...
export class RSAKey extends Key {
  ...
  static fromPublicKey(keyInput: KeyInputs.RSAPublicKeyInput): RSAKey {
    if ("key" in keyInput) {
      ...
      let format =
        keyInput.key.indexOf("-----BEGIN RSA PUBLIC KEY-----") > -1
          ? "pkcs1-public-pem"
          : "pkcs8-public-pem";
  ...
  static fromPrivateKey(keyInput: KeyInputs.RSAPrivateKeyInput): RSAKey {
    if ("key" in keyInput) {
      let rsaKey = new NodeRSA();
      let format =
        keyInput.key.indexOf("-----BEGIN RSA PRIVATE KEY-----") > -1
          ? "pkcs1-private-pem"
          : "pkcs8-private-pem";
      rsaKey.importKey(keyInput.key, format);
      let n = base64url.encode(rsaKey.keyPair.n.toBuffer().slice(1));
   ...
   */
  static fromKey(
    keyInput: KeyInputs.RSAPublicKeyInput | KeyInputs.RSAPrivateKeyInput
  ): RSAKey {
    if (this.isPrivateKeyInput(keyInput))
      return this.fromPrivateKey(keyInput as KeyInputs.RSAPrivateKeyInput);
  ...
  static isPrivateKeyInput(
    ...
    if ("key" in keyInput) {
      ...
      if (publicKeyObject.e && publicKeyObject.n) {
        return false;
      }
  ...
  toJWK(
    privateKey?: boolean
  ): KeyObjects.RSAPrivateKeyObject | KeyObjects.RSAPublicKeyObject {
    if (privateKey) {
      if (this.private) {
        return {
          kty: this.kty,
  ...
  exportKey(format: KEY_FORMATS): string {
    switch (format) {
      case KEY_FORMATS.PKCS1_PEM:
        return this.toPEM("pkcs1");
      case KEY_FORMATS.PKCS8_PEM:
        return this.toPEM("pkcs8");
...
export class ECKey extends Key {
  ...
  static fromPublicKey(keyInput: KeyInputs.ECPublicKeyInput): ECKey {
    if ("key" in keyInput) {
      ...
      try {
        switch (keyInput.format) {
          case KEY_FORMATS.BASE58:
            ...
            break;
          case KEY_FORMATS.BASE64:
            key_buffer = base64url.toBuffer(base64url.fromBase64(keyInput.key));
            break;
          case KEY_FORMATS.HEX:
      ...
      let x = base64url.encode(
        ellipticKey.getPublic().getX().toArrayLike(Buffer)
      );
      let y = base64url.encode(
        ellipticKey.getPublic().getY().toArrayLike(Buffer)
      );
      return new ECKey(
        keyInput.kid,
        KTYS.EC,
   ...
   */
  static fromKey(
    keyInput: KeyInputs.ECPublicKeyInput | KeyInputs.ECPrivateKeyInput
  ): ECKey {
    if (this.isPrivateKeyInput(keyInput))
      return this.fromPrivateKey(keyInput as KeyInputs.ECPrivateKeyInput);
  ...
  static isPrivateKeyInput(
    ...
    if ("key" in keyInput) {
      ...
      if (privateKeyObject.d && privateKeyObject.x && privateKeyObject.y) {
        return true;
      }

      let publicKeyObject = keyInput as KeyObjects.ECPublicKeyObject;
      if (publicKeyObject.x && publicKeyObject.y) {
        return false;
      }
  ...
  toJWK(
    privateKey?: boolean
  ): KeyObjects.ECPrivateKeyObject | KeyObjects.ECPublicKeyObject {
    if (privateKey) {
      if (this.private) {
        return {
          kty: this.kty,
  ...
  exportKey(format: KEY_FORMATS): string {
    ...
    if (this.private) {
      keyString = ec
        .keyFromPrivate(base64url.toBuffer(this.d || " "))
        .getPrivate()
        .toArrayLike(Buffer);
    } else {
      let pub = {
      ...
      keyString = Buffer.from(
        ec.keyFromPublic(pub).getPublic().encode("hex", false),
        "hex"
      );
    }
    ...
    switch (format) {
      case KEY_FORMATS.HEX:
        return keyString.toString("hex");
      case KEY_FORMATS.BASE58:
        return base58.encode(keyString);
      case KEY_FORMATS.BASE64:
        return keyString.toString("base64");
      case KEY_FORMATS.BASE64URL:
        return base64url.encode(keyString);
      case KEY_FORMATS.PKCS1_PEM:
      case KEY_FORMATS.PKCS8_PEM:
      default:
        throw new Error(ERRORS.INVALID_KEY_FORMAT);
...
export class OKP extends Key {
   ...
   */
  static fromKey(
    keyInput: KeyInputs.OKPPublicKeyInput | KeyInputs.OKPPrivateKeyInput
  ): OKP {
    if (this.isPrivateKeyInput(keyInput))
      return this.fromPrivateKey(keyInput as KeyInputs.OKPPrivateKeyInput);
  ...
  static isPrivateKeyInput(
    ...
    if ("key" in keyInput) {
      ...
      if (privateKeyObject.d && privateKeyObject.x) {
        return true;
      }
  ...
  toJWK(
    privateKey?: boolean
  ): KeyObjects.OKPPrivateKeyObject | KeyObjects.OKPPublicKeyObject {
    if (privateKey) {
      if (this.private) {
        return {
          kty: this.kty,
  ...
  exportKey(format: KEY_FORMATS): string {
    ...
    if (this.private) {
      keyString = ed
        .keyFromSecret(base64url.toBuffer(this.d || " "))
        .getSecret();
    } else {
...
export class KeySet {
      ...
      switch (key.kty) {
        case KTYS[KTYS.RSA]: {
          newKeySet.push(
            RSAKey.fromKey(
              key as
                | KeyObjects.RSAPrivateKeyObject
                | KeyObjects.RSAPublicKeyObject
            )
          );
          break;
        }
        case KTYS[KTYS.EC]: {
          newKeySet.push(
            ECKey.fromKey(
              key as
                | KeyObjects.ECPrivateKeyObject
                | KeyObjects.ECPublicKeyObject
            )
          );
          break;
        }
        case KTYS[KTYS.OKP]: {
          newKeySet.push(
            OKP.fromKey(
              key as
                | KeyObjects.OKPPrivateKeyObject
                | KeyObjects.OKPPublicKeyObject
            )
          );
          break;
        }
        default:
          throw new Error(ERRORS.INVALID_KEY_SET);
      }
    });
  ...
  getKey(kid: string): Key[] {
    let keys = this.ketSet.filter((k) => {
      return k.checkKid(kid);
    });
  ...
  addKey(key: KeyObjects.BasicKeyObject) {
    if (
      this.ketSet.filter((k) => {
        return k.checkKid(key.kid);
      }).length === 0
    ) {
      switch (key.kty) {
        case KTYS[KTYS.RSA]: {
  ...
  removeKey(kid: string) {
    this.ketSet = this.ketSet.filter((key) => {
      return !key.checkKid(kid);
    });
  }
