...
import { getBasicJWT, getModifiedJWT, getModifiedJWTSigned, claims } from './common.testdata';
...
const jwtBasicGoodDecoded = getBasicJWT(testDidDoc.verificationMethod[1].id, testDID, testDID);
...
const signedJWTs = {
    ...
    bad: {
        jwtBadNoKid: getModifiedJWTSigned(TD_BASIC_JWT.decoded, siginingInfo, false, 'kid'),
        jwtBadNoIss: getModifiedJWTSigned(TD_BASIC_JWT.decoded, siginingInfo, true, 'iss'),
        jwtBadNoScope: getModifiedJWTSigned(TD_BASIC_JWT.decoded, siginingInfo, true, 'scope'),
...
export const requestJWT = {
    good: {
        ...
        withVPToken: getModifiedJWT(TD_BASIC_JWT.decoded, true, 'claims', claims.good)
    },
    bad: {
        withVPToken: getModifiedJWT(TD_BASIC_JWT.decoded, true, 'claims', claims.bad)
    }
};
