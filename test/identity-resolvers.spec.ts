...
import { didDocs, resolvers } from "./data/identity-resolvert.testdata";

//Set the default timeout interval to 17000 ms for all tests and before/after hooks
jest.setTimeout(17000);

describe.each(didDocs)("($tag)", ({ tag, didDocument, keys }) => {
...
describe("001.02 CombinedDidResolver Functionalities with given Resolver ", function () {
  describe.each(resolvers)("($tag)", ({ resolver, didDoc }) => {
    test("a. ability to add EthrDidResolver and resolve a DID", async () => {
      let did = didDoc.didDocument.id;
  ...
  test("c. ability to add multiple resolvers and resolve a DID", async () => {
    ...
    for (const r of resolvers) {
      const resolvedDIDKey = await combinedDidResolver.resolveDidDocumet(
        r.didDoc.didDocument.id
      );
      expect(resolvedDIDKey.id).toEqual(r.didDoc.didDocument.id);
    }
  });
});
