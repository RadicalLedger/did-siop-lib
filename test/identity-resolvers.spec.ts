...
import { didDocs } from "./data/identity-resolvert.testdata";

//Set the default timeout interval to 17000 ms for all tests and before/after hooks
jest.setTimeout(17000);

describe.each(didDocs.all)("($tag)", ({ tag, didDocument, keys }) => {
...
describe("001.02 CombinedDidResolver Functionalities with given Resolver ", function () {
  test("a. ability to add EthrDidResolver and resolve a DID", async () => {
    let did = didDocs.ethrDidDoc.didDocument.id;
  ...
  test("b. ability to add KeyDidResolver and resolve a DID", async () => {
    let did = didDocs.keyDidDoc.didDocument.id;
    let combinedDidResolver = new CombinedDidResolver("");
  ...
  test("c. ability to add multiple resolvers and resolve a DID", async () => {
    ...
    const resolvers = didDocs.resolvers;
    resolvers.forEach((resolver) => combinedDidResolver.addResolver(resolver));
